...
mass = m.Var(value=4821, lb = 1000) #lb, ub are upper and lower bounds respectively
theta = m.Var()
#theta.STATUS = 1
...
m.Equation(y.dt()==tf*ydot) #Expression for velocity
m.Equation(ydot.dt() == tf*ydoubledot) #Expression for acceleration

m.Equation(mass.dt() == -M_dot*tf) #Expression for mass


m.Equation(ydoubledot == (Ft/mass)*m.cos(theta) - 1.62)

#m.Equation(x.dt()==tf*xdot) #Expression for velocity
#m.Equation(xdot.dt() == tf*xdoubledot) #Expression for acceleration
#m.Equation(xdoubledot == (Ft/mass)*m.sin(theta))
...
m.fix(y, pos=0,val=R0) #Initial x position
m.fix(theta, pos=0, val=0)
...
constraint_2 [-1] = 1
final_ydot = m.Param(value = constraint_2 )
#Creating a way to check if time is final:
m.Minimize(ydot*final_ydot)
