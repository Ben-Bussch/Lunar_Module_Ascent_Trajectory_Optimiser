...
m.Equation(ydoubledot == ( ( (Ft/((M0-mass_scalar*mass)*((x*Scalar)**2 + (y*Scalar)**2)**(1/2)))*\
                          ((y*Scalar)*m.cos(3*angle)+(x*Scalar)*m.sin(3*angle)) )\
           - (y*Scalar)*(G*M/(((x*Scalar+R0)**2 + (y*Scalar+R0)**2)**(3/2))) )/ Scalar\
           )


m.Equation(xdoubledot == ( ( (Ft/((M0-mass_scalar*mass)*((x*Scalar)**2 + (y*Scalar)**2)**(1/2)))*\
                          ((x*Scalar)*m.cos(3*angle)-(y*Scalar)*m.sin(3*angle)) )\
           - (x*Scalar)*(G*M/(((x*Scalar+R0)**2 + (y*Scalar+R0)**2)**(3/2))))/ Scalar\
           )
...
m.Equation(ydot*final_velocity*Scalar <= 1) #Ensures minimal vertical component of velocity
...
y_a_list = [0]*len(x.value)
x_a_list = [0]*len(x.value)
for i in range(len(x.value)):
    x_pos_list[i] = x.value[i]*-86904
    y_pos_list[i] = y.value[i]*86904
    x_a_list[i] = xdoubledot.value[i]*-86904
    y_a_list[i] = ydoubledot.value[i]*86904
