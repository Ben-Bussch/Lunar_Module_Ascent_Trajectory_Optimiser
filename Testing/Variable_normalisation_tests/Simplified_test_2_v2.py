...
m.options.OTOL = 1e-6 #allows for margins of error in solution, default of 1e-6
m.options.RTOL = 1e-6 #same as above
...
#system dynamics
m.Equation(ydoubledot == ((Ft/(M0-mass_scalar*mass))*m.cos(3*angle)-G*M/(y*Scalar+R0)**2)/Scalar)
m.Equation(xdoubledot == ((Ft/(M0-mass_scalar*mass))*m.sin(3*angle)/Scalar))
...
for i in range(len(x.value)):
    ...
    y_pos_list[i] = y.value[i]*86904+R0
    x_a_list[i] = xdot.value[i]*86904
    y_a_list[i] = ydot.value[i]*86904
...
ax.set_aspect('equal')
ax.grid()
plt.savefig('flatmoon_position', dpi=300)
...
plt.ylabel('y velocity / meters per second')
ax.set_xlabel('x velocity / meters per second')
ax.grid()
plt.savefig('flatmoon_velocity.png', dpi=300)
...
ax.grid()
plt.savefig('flatmoon_angle', dpi=300)
