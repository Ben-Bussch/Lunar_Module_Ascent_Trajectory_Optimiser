...
angle.REQONCTRL = 1 #tells solver whether to change MVs or run as simulator
...
m.Equation(ydoubledot == ( ( (Ft/((M0-mass_scalar*mass)*(((x*Scalar-x_offset)**2 + (y*Scalar+y_offset)**2)**(1/2))))*\
                          ((y*Scalar+y_offset)*m.cos(3*angle)+(x*Scalar-x_offset)*m.sin(3*angle)) )\
           - ((y*Scalar+y_offset)*(GM/(((x*Scalar-x_offset)**2 + (y*Scalar+y_offset)**2)**(3/2)))) )/ Scalar\
           )


m.Equation(xdoubledot == ( ( (Ft/((M0-mass_scalar*mass)*(((x*Scalar-x_offset)**2 + (y*Scalar+y_offset)**2)**(1/2))))*\
                          ((x*Scalar-x_offset)*m.cos(3*angle)-(y*Scalar+y_offset)*m.sin(3*angle)) )\
           - (x*Scalar-x_offset)*(GM/(((x*Scalar-x_offset)**2 + (y*Scalar+y_offset)**2)**(3/2))))/ Scalar\
           )
...
m.Equation(((y*Scalar+y_offset)**2+(x*Scalar+x_offset)**2)+final_radius >= (hlow)**2) #ensures final radius is greater or equal to rfmin
#m.Equation(y+final_radius >= 0.1)
...
f_y = y.value[-1]*86904+y_offset
f_x = x.value[-1]*-86904-x_offset
