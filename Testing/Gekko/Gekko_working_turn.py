...
m.options.OTOL = 1e-1 #Default of 1e-6, but should be a helping start
m.options.RTOL = 1e-1
...
theta = m.MV(name='theta', lb=-1, ub = 1, value=1) #May require to be changed to MV
#thetadot = m.MV(name='thetadot') 

theta.STATUS = 1 #Allows computer to change it
#theta.DCOST = 1e-5
theta.REQONCTRL = 1
...
#System dynamics:
m.Equation(ydoubledot == (Ft/mass)*theta-((G*M)/(y**2)))    


#Constraints

#m.fix(y,val=R0, pos=0)      #Initial y position
#m.fix(theta,val=3.14159265359/2, pos=len(m.time)-1)    #Initial y velocity
m.Equation(y >= R0)
#m.Equation(m.cos(theta) >= 0.2)
...
m.Equation(ydot*final_velocity <= 50)
m.Equation(theta*final_velocity <= 0)
